...
func Test_MainRouter(t *testing.T) {
	...
	doRequest(t, router, "POST", "/api/login", "", `{"login": "unknownuser","password": "password"}`, http.StatusBadRequest, `User not found`)
	// Do a login with a known user but bad password
	doRequest(t, router, "POST", "/api/login", "", `{"login": "admin","password": "badpassword"}`, http.StatusBadRequest, `User not found`)
	...
	doRequest(t, router, "POST", "/api/common/getsharetoken", "", shareTokenTargetPath, http.StatusUnauthorized, "no token found")

	// === Try to access the resources as an normal user ===
	userHeader := "Bearer " + doRequest(t, router, "POST", "/api/login", "", `{"login": "user","password": "password"}`, http.StatusOK, `eyJhbG`)
	...
	doRequest(t, router, "POST", "/api/common/getsharetoken", userHeader, "", http.StatusBadRequest, `Path cannot be empty, and must began with /api/files`)
	// Try to get a share token for an user reserved resource
	shareHeader := "Bearer " + doRequest(t, router, "POST", "/api/common/getsharetoken", userHeader, shareTokenTargetPath, http.StatusOK, `eyJhbG`)
	...
	doRequest(t, router, "GET", "/api/files/usersrw/File users 02.txt", shareHeader, "", http.StatusForbidden, "The share token can only be used for the given path")
	// Try to get a share token for an admin reserved resource (it's possible but the token will not be usable)
	shareHeader = "Bearer " + doRequest(t, router, "POST", "/api/common/getsharetoken", userHeader, "/api/files/adminsrw/File admins 01.txt", http.StatusOK, `eyJhbG`)
	// Try to use the share token for the admin token path
	doRequest(t, router, "GET", "/api/files/adminsrw/File admins 01.txt", shareHeader, "", http.StatusForbidden, "User has role user, which is not in allowed roles ([admin])")

	// === Try to access the resources as an admin ===
	// Do a login with the correct admin user
	adminHeader := "Bearer " + doRequest(t, router, "POST", "/api/login", "", `{"login": "admin","password": "password"}`, http.StatusOK, `eyJhbG`)
	...
	doRequest(t, router, "POST", "/api/login", "", `{"login": "admin","password": "password"}`, http.StatusBadRequest, `User not found`)
